...
class room(object):
    def __init__(self, lists, index, agentrep):
        ...
        self.thermostats = []
        self.lcd = []
        self.objlists = [self.regulators, self.agents, self.powerSs, self.lights, self.windows, self.irs, self.sensors,
                         self.adbs, self.thermostats, self.lcd]
        ...
        for self._object in self._objects:  # _window1=['window','window1',1]
            ...
            self._object['preStr'] = None  # reset to avoid false assignment in case of unrecognised objects
            self.preId = None
            self.objClass = None
            ...
            self.objlists[self.preId].append(
                self.objClass(self, self._object, self.preId, str(len(self.objlists[self.preId]))))
        #			except TypeError:
        #				logger.info('unknown')
...
class brightness_regulator(object):
        ...
        if self.enabled == 1 and not self.brightness_block and self.summ != 0:
            ...
            if abs(self.brightness_diff) > self._brightness_tolerance:
                ...
                if self.lightweight != 0:
                    ...
                    for obj in self.lights:
                        logger.debug(
                            'actual:' + str(self._brightness_actual) + 'target:' + str(self._brightness_target))
    ...
    def informAll(self):
        self.msg = '0,0,' + str(self.enabled) + ',' + str(self._brightness_actual) + ',' + str(
            self._brightness_target) + ',' + self.mode
    ...
    def aProcessor(self, _input):  # try me
        #		self.brightness_actual=int(_input[2])
        #		self.informAll() ###############################suspended till fixing the range
        pass
