...
class rgb(obj):
    def __init__(self, parent, info, preId, objid):
        ...
        self.R = light(self, dict(name=self.name + '-R', agent=self.info['agent'], agent_index=self.info['agent_index'],
                                  preStr='l', ps=0), 0, 0, sub=True)
        self.G = light(self,
                       dict(name=self.name + '-G', agent=self.info['agent'], agent_index=self.info['agent_indexG'],
                            preStr='l', ps=0), 0, 1, sub=True)
        self.B = light(self,
                       dict(name=self.name + '-R', agent=self.info['agent'], agent_index=self.info['agent_indexB'],
                            preStr='l', ps=0), 0, 2, sub=True)
    ...
    def compose(self, to):
        if to == 'agentS':  # status request
            ...
            msg = 'obj,' + self.indexR + ',' + str(self.cmdid) + ',' + str(self.tR) + ',' + str(self.tG) + ',' + str(
                self.tB)
        elif to == 'client':
            msg = '3,' + str(self.objid) + ',' + str(self.R.isActive) + ',' + str(self.aR) + ',' + str(
                self.aG) + ',' + str(
                self.aB)  # using isActive Value from any child light is much easier than having an own value
        return msg
